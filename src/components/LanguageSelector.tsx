...
export default function LanguageSelector() {
  ...
  const setInterpretLanguage = useDreamStore((s) => s.setInterpretLanguage)
  ...
  const changeLanguage = (langCode: string) => {
    ...
    localStorage.setItem('oneiro_language', code)
    localStorage.setItem('i18nextLng', code)
    setInterpretLanguage(code)
          ...
          >
            {LANGUAGES.map((lang) => (
              <button
                key={lang.code}
                type="button"
                onPointerDown={(e) => { e.preventDefault(); e.stopPropagation(); changeLanguage(lang.code) }}
